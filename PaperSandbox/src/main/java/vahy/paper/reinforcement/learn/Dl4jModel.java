...
public class Dl4jModel implements SupervisedTrainableModel {
    ...
    public Dl4jModel(int inputDimension, int outputDimension, List<Integer> hiddenLayerSizeList, long seed, double learningRate, int trainingEpochCount) {
        ...
        this.policyArray = Nd4j.create(new int[] {outputDimension - NodeEvaluator.POLICY_START_INDEX});
...
//                SECOND_HIDDEN_NAME)
            ...
            new OutputLayer.Builder()
                ...
                .nIn(SECOND_HIDDEN_SIZE)
//                .nIn(inputDimension)
                .nOut(outputDimension - NodeEvaluator.POLICY_START_INDEX)
...
//            INPUT_NAME)
    ...
    public void fit(double[][] input, double[][] target) {
        MultiDataSetIterator iterator = new MultiDataSetIterator() {
            ...
            public MultiDataSet next() {
                ...
                for (int i = 0; i < outputDimension - NodeEvaluator.POLICY_START_INDEX; i++) {
                    policyArray.put(0, i, target[dataIndex][i + NodeEvaluator.POLICY_START_INDEX]);
                }
                qValueArray.put(0, 0, target[dataIndex][NodeEvaluator.Q_VALUE_INDEX]);
                riskValueArray.put(0, 0, target[dataIndex][NodeEvaluator.RISK_VALUE_INDEX]);
    ...
    public double[] predict(double[] input) {
        ...
        outputVector[NodeEvaluator.Q_VALUE_INDEX] = qValueArray[0];
        outputVector[NodeEvaluator.RISK_VALUE_INDEX] = riskValueArray[0];
        System.arraycopy(probabilities, 0, outputVector, NodeEvaluator.POLICY_START_INDEX, outputVector.length - NodeEvaluator.POLICY_START_INDEX);
        return outputVector;
    }
    ...
    public double[][] predict(double[][] input) {
        ...
        if(qValueArray[0].length != NodeEvaluator.POLICY_START_INDEX) {
            throw new IllegalStateException("Qvalue Vector cannot have length different from 1");
        }
        if(riskValueArray[0].length != NodeEvaluator.POLICY_START_INDEX) {
            throw new IllegalStateException("Risk Vector cannot have length different from 1");
        }
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            outputMatrix[i][NodeEvaluator.Q_VALUE_INDEX] = qValueArray[i][0];
            outputMatrix[i][NodeEvaluator.RISK_VALUE_INDEX] = riskValueArray[i][0];
            System.arraycopy(probabilities[i], 0, outputMatrix[i], NodeEvaluator.POLICY_START_INDEX, probabilities.length - NodeEvaluator.POLICY_START_INDEX);
        }
        return outputMatrix;
    }
